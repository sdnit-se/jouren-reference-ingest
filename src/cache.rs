...
pub struct Cache { per_sensor: usize, max_sensors: usize, inner: Mutex<Inner> }

#[derive(Default)]
struct Inner { by_sensor: HashMap<SensorId, Entry>, readings: usize, bytes: usize }

struct Entry { readings: VecDeque<Reading>, newest: time::OffsetDateTime }
    ...
    pub fn insert<'a>(&self, readings: impl IntoIterator<Item = (&'a SensorId, &'a Reading)>) {
        ...
        for (sensor, reading) in readings {
            if !inner.by_sensor.contains_key(sensor) {
                ...
                inner.by_sensor.insert(sensor.clone(), Entry { readings: VecDeque::with_capacity(self.per_sensor.min(64)), newest: reading.ts });
            }
            let entry = inner.by_sensor.get_mut(sensor).expect("entry inserted");
            let position = entry.readings.partition_point(|existing| existing.ts <= reading.ts);
            entry.readings.insert(position, reading.clone());
            entry.newest = entry.newest.max(reading.ts);
            let evicted = if entry.readings.len() > self.per_sensor { entry.readings.pop_front() } else { None };
            ...
            while inner.by_sensor.len() > self.max_sensors {
                let victim = inner.by_sensor.iter().min_by_key(|(_, entry)| entry.newest).map(|(sensor, _)| sensor.clone()).expect("cache is non-empty");
                let removed = inner.by_sensor.remove(&victim).expect("victim exists");
                inner.bytes -= victim.byte_len() + removed.readings.iter().map(Reading::byte_len).sum::<usize>();
...
mod tests {
    ...
    fn keeps_only_the_newest_per_sensor() {
        let cache = Cache::new(3, 10); let sensor = SensorId::parse("s-1").unwrap(); let readings: Vec<_> = (1..=5).map(reading).collect();
        cache.insert(readings.iter().map(|r| (&sensor, r))); let latest = cache.latest(&sensor, 10).unwrap();
        assert_eq!(latest.iter().map(|r| r.value).collect::<Vec<_>>(), vec![5.0, 4.0, 3.0]); assert_eq!(cache.stats().readings, 3);
    }

    #[test]
    fn bounds_sensor_cardinality_and_keeps_newest_sensor_data() {
        let cache = Cache::new(2, 2); let a = SensorId::parse("a").unwrap(); let b = SensorId::parse("b").unwrap(); let c = SensorId::parse("c").unwrap();
        cache.insert([(&a, &reading(1)), (&b, &reading(2)), (&c, &reading(3))]);
        assert_eq!(cache.stats().sensors, 2); assert!(cache.latest(&c, 1).is_some()); assert!(cache.latest(&a, 1).is_none());
    }

    #[test]
    fn historical_warm_up_does_not_evict_newer_live_sensor() {
        let cache = Cache::new(3, 2); let live = SensorId::parse("live").unwrap(); let old = SensorId::parse("old").unwrap(); let other = SensorId::parse("other").unwrap();
        cache.insert([(&live, &reading(100)), (&other, &reading(90))]);
        let historical: Vec<_> = (1..10).map(reading).collect(); cache.insert(historical.iter().map(|r| (&old, r)));
        assert!(cache.latest(&live, 1).is_some()); assert_eq!(cache.latest(&live, 1).unwrap()[0].value, 100.0);
    }
}
